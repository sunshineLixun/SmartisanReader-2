...
extension ViewController{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "\(tableView.classForCoder)") {
            ...
            let cell = UITableViewCell.init(style: .default, reuseIdentifier: "\(tableView.classForCoder)")
            let asyncView = AsyncRichTextView.init(CGRect.init(x: 0, y: 0, width: (cell.bounds.size.width), height: layouts[indexPath.row].height), layout: layouts[indexPath.row])
            cell.contentView.addSubview(asyncView)
            return cell
        }
    }
